...
class _Pill extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final String? actionLabel = snack.actionLabel;
    final VoidCallback? onAction = snack.onAction;
    final bool hasAction = actionLabel != null && onAction != null;
    final bool ltr = Directionality.of(context) == TextDirection.ltr;

    final Color background = snack.backgroundColor ?? cs.inverseSurface;
    final Color foreground = snack.foregroundColor ?? cs.onInverseSurface;
    final Color chip =
        snack.iconBackgroundColor ??
        (snack.backgroundColor == null ? cs.inversePrimary : foreground);

    // The spec's 48dp single line container height: 8dp around the 32dp
    // chip or action button, or 14dp around the 20dp body medium line when
    // the pill is text only.
    final double vertical = (icon != null || hasAction) ? 8 : 14;
            ...
            children: [
              if (icon != null) ...[
                Container(
                  ...
                  child: Icon(icon, size: 18, color: background),
                ),
